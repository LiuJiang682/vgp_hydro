...
public class VgpHydroObservationsDaoImpl implements VgpHydroObservationsDao {
	...
	public boolean updateOrSave(Entity entity) {
		...
		int row = jdbcTemplate.update(INSERT_SQL,
				new Object[] { observation.getId(), observation.getLoaderId(),
						...
						observation.getFileName(), observation.getRowNumber(),
	...
	public void printTableColumns() {
		...
		tableMetaDataContext.setTableName("OBSERVATIONS");
		tableMetaDataContext.processMetaData(jdbcTemplate.getDataSource(), 
				Collections.<String>emptyList(), new String[0]);
		tableMetaDataContext.getTableColumns().stream()
